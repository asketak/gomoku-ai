...
class ai():
    ...
    def generatemy(self, width):  # my matrix, star
        self.pl_weight_board = [
            [0 for i in xrange(width)] for j in xrange(width)]
        for x in range(width):
            for y in range(width):
                # funguje jen na ctvrecich
                if x == width / 2 or y == width / 2 or x + y == width - 1 or abs(x) == abs(y):
                    self.pl_weight_board[x][y] = - self.weights_width / \
                        (1 + max(abs(x - width / 2), abs(y - width / 2)))
    ...
    def generateopp(self, width):
        self.opp_weight_board = [
            [0 for i in xrange(width)] for j in xrange(width)]
        for x in range(width):
            for y in range(width):
                # funguje jen na ctvrecich
                if x == width / 2 or y == width / 2 or x + y == width - 1 or abs(x) == abs(y):
                    self.opp_weight_board[x][y] = self.weights_width / \
                        (1 + max(abs(x - width / 2), abs(y - width / 2)))
    ...
    def generateinit(self, width):
        ...
        for x in range(self.width):
            for y in range(self.width):
                self.init_weigth[x][y] = (
                    (abs(x - width / 2) + abs(y - width / 2))) / float(width)
        self.init_weigth = np.around(self.init_weigth, decimals=1)
        # pprint(self.init_weigth)
